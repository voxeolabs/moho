...
public class RemoteLocalJoinDelegate extends JoinDelegate implements
		MediaEventListener<SdpPortManagerEvent> {
	private static final Logger LOG = Logger
			.getLogger(LocalRemoteJoinDelegate.class);
	...
	public RemoteLocalJoinDelegate(
			final RemoteParticipantImpl remoteParticipant,
			final Participant localParticipant, final Direction direction,
			byte[] offer) {
	...
	public void doJoin() throws Exception {
		try {
			...
			((ParticipantContainer) _localParticipant).startJoin(
					_remoteParticipant, this);

			if (_localParticipant.getMediaObject() == null
					&& _localParticipant instanceof Call) {
				notifyRemote = true;
			...
			this.createNetworkConnection(
					_localParticipant.getApplicationContext(),
					_remoteParticipant);

			// 2 join network connection
			((Joinable) _localParticipant.getMediaObject()).join(_direction,
					_remoteParticipant.getNetworkConnection());

			// process sdp offer.
			_remoteParticipant.getNetworkConnection().getSdpPortManager()
	...
	public void onEvent(SdpPortManagerEvent event) {
		if (event.getEventType().equals(SdpPortManagerEvent.ANSWER_GENERATED)) {
			if (event.isSuccessful()) {
				...
				try {
					_remoteParticipant.remoteJoinAnswer(
							_localParticipant.getRemoteAddress(), sdp);
	...
	protected synchronized void createNetworkConnection(
			ApplicationContext context, RemoteParticipantImpl remoteParticipant)
			throws MsControlException {
		MsControlFactory mf = context.getMSFactory();

		remoteParticipant.setMediaSession(mf.createMediaSession());

		NetworkConnection network = remoteParticipant.getMediaSession()
				.createNetworkConnection(NetworkConnection.BASIC, null);
	...
	public synchronized void done(Cause cause, Exception exception) {
		...
		((ParticipantContainer) _localParticipant).joinDone(_remoteParticipant,
				this);

		JoinCompleteEvent joinCompleteEvent = new MohoJoinCompleteEvent(
				_localParticipant, _remoteParticipant, cause, exception, false);
		if (cause == Cause.JOINED) {
			((ParticipantContainer) _localParticipant).addParticipant(
					_remoteParticipant, _joinType, _direction, null);
		}
