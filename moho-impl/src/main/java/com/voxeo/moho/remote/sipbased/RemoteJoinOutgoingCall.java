...
public class RemoteJoinOutgoingCall extends SIPOutgoingCall implements RemoteParticipant {

  private static final Logger LOG = Logger.getLogger(RemoteJoinOutgoingCall.class);
  ...
  public RemoteJoinOutgoingCall(ExecutionContext context, SIPEndpoint from, SIPEndpoint to, Map<String, String> headers) {
    super(context, from, to, headers);
    if (_headers == null) {
      _headers = new HashMap<String, String>();
    }
    joiner = from.getSipURI();
    joinee = to.getSipURI();
  }
  ...
  public Unjoint unjoin(final Participant other, final boolean isInitiator) {
    Unjoint task = new UnjointImpl(_context.getExecutor(), new Callable<UnjoinCompleteEvent>() {
      ...
      public UnjoinCompleteEvent call() throws Exception {
        ...
        RemoteJoinOutgoingCall.this.disconnect();
        return event;
      }
    });
  ...
  public int hashCode() {
    final int prime = 31;
    int result = 1;
    result = prime * result + ((joinee.getUser() == null) ? 0 : joinee.getUser().hashCode());
    return result;
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof RemoteParticipantImpl) {
      ...
      if (this.getJoinee().getUser().equalsIgnoreCase(other.getId())) {
        return true;
      }
      else {
        return false;
